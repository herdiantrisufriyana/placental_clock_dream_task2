...
```{r figure-s7, echo=FALSE, fig.height=2, fig.width=12}
figure_s7
```

```{r Save figure-s7, eval=FALSE, include=FALSE}
ggsave("doc/figure2b.eps", height = 2, width = 12, units = "in")
ggsave("doc/figure2b.svg", height = 2, width = 12, units = "in")
```
...
stratified_plot(
  ga_res_conds_pred_450k_est_eval_plot, "Res-Conds-GA† (450k)"
  ,ga_res_conds_pred_est_eval_plot, "Res-Conds-GA†"
  ,ga_res_comb_450k_est_eval_plot, "Res-Comb-GA (450k)"
  ,ga_res_comb_est_eval_plot, "Res-Comb-GA"
)
```
...
df_train0 |>
  ...
  write_csv("input/Leaderboard_beta_subchallenge2.csv")
```
...
if (!dir.exists("pcd2_sub3/data")) {
  dir.create("pcd2_sub3/data", recursive = TRUE)
}

if (!dir.exists("pcd2_sub3/inst/extdata")) {
  dir.create("pcd2_sub3/inst/extdata", recursive = TRUE)
}
```

```{r ResComb-GA prediction - Copy the files, eval=FALSE, include=FALSE}
pcd2_sub3_files_non_default <-
...
pcd2_sub3_files_non_default |>
  file.copy(paste0("pcd2_sub3/", pcd2_sub3_files_non_default))
```
...
cd pcd2_sub3
docker buildx build --platform linux/amd64 -t docker.synapse.org/syn62407322/ga-res-comb-est-850k:v1.0 --push .
cd ..
```
